...
exports = module.exports = function(req, res) {
	...
	function printActivity () {
		...
		db.chat.find({ channel: '#uwec-cs' }).sort({ time: -1}).limit(20).all(function (err, docs) {
			if (docs && docs.length) {
				var i = 0;
				var day = null;
				res.write('<table>\n');
				docs.reverse().forEach(function (doc) {
					...
					res.write('<tr class="' + className + '"><td valign="top">' + moment(doc.time).format('HH:mm:ss') + '</td><td valign="top" class="nick"><nobr>' + doc.nick + '</nobr></td><td valign="top">' + entities.encode(doc.message) + '</td></tr>\n');
				});
				res.write("</table>\n");
			} else {
		...
		db.confessions.sort({time: -1}).limit(20).all(function (err, docs) {
			if (docs && docs.length) {
				...
				docs.reverse().forEach(function (doc) {
					...
					var className = (++i & 1) ? 'odd' : 'even';
					res.write('<tr class="' + className + '"><td valign="top">' + moment(doc.time).format('HH:mm:ss') + '</td><td valign="top">' + entities.encode(doc.confession) + '</td></tr>\n');
				});
				res.write("</table>\n");
			} else {
